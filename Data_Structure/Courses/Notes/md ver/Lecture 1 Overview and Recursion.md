...
- Value $b$ to be searched for
Output:
- If $b$ if found, return the corresponding index
- If not, return -1

Method is recursive:
- Compare $b$ with `X[mid]`
- If $b=$`x[mid]`, return mid.
- If less, find in the left half.
- If larger, find in the right half.