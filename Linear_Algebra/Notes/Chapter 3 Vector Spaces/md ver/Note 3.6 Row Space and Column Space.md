...
When $\mathbf{b}=\mathbf{0}$, we can know $A\mathbf{x}=\mathbf{0}$ will have ==only the trivial solution== if and only if the column vectors of $A$ are ==linearly independent== since 
$$
x_{1}\mathbf{a}_{1}+x_{2}\mathbf{a}_{2}+\cdots+x_{n}\mathbf{a}_{n}=\mathbf{0}
$$
are zero for all scalars.
## Theorem 3.6.3
...
Then we can get the same for the transpose of $A$, denoted by $A^{T}$, such that 
