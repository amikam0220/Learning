...
#### Proof:
Let $\mathbf{u}_{1},\mathbf{u}_{2},\cdots,\mathbf{u}_{m}$ be the vectors in $V$, where $m>n$. Then they can be represented as 
...
Then the linear combination of $\mathbf{u}_{1},\mathbf{u}_{2},\cdots,\mathbf{u}_{m}$ equals to zero is 

$$
c_{1}\mathbf{u}_{1}+c_{2}\mathbf{u}_{2}+\cdots+c_{m}\mathbf{u}_{m}=\mathbf{0}
$$

Substitution, we get 

$$
\sum_{k=1}^{m} \left(c_{k}\cdot \sum_{j=1}^{n} a_{kj}\mathbf{v}_{j}\right)
=\sum_{j=1}^{n} \left( \sum_{k=1}^{m} a_{kj}c_{k} \right)\mathbf{v}_{j}=\mathbf{0}
$$
