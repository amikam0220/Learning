...
$$v_{t}=\frac{L}{t}
$$
​, where $L$ is distance between marks A and B, and $t$ is average time for the sphere to fall from A to B (A is far below the surface to ensure $v_{t}$ is reached; B is not near the bottom). Then we can obtain the equation of $\eta$: 
$$
...
Since the fluid is confined, vessel walls affect motion. The corrected formula accounts for this:
$$
\eta=
\frac{(\rho-\sigma)g}{18L}d^{2}t
\left( 1-2.4 \frac{d}{D}-3.3 \frac{d}{2h} \right)
$$
where $D$ is the inner diameter of the vessel, and $h$ is the height of castor oil .
